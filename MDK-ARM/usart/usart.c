...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
	...
	{
		if((USART_RX_STA_1&0x8000)==0)//接收未完成
		{
			if(USART_RX_STA_1&0x4000)//接收到了0x0d
			{
				if(aRxBuffer_1[0]!=0x0a)USART_RX_STA_1=0;//接收错误,重新开始
				else USART_RX_STA_1|=0x8000;	//接收完成了 
			}
			else //还没收到0X0D
			{	
				if(aRxBuffer_1[0]==0x0d)USART_RX_STA_1|=0x4000;
				else
				{
					Rx_buff_1[USART_RX_STA_1&0X3FFF]=aRxBuffer_1[0] ;
					USART_RX_STA_1++;
					if(USART_RX_STA_1>(USART_REC_LEN_1-1))USART_RX_STA_1=0;//接收数据错误,重新开始接收	  
				}		 
			}
		}
...
void Uart_test(void)
{
		if(USART_RX_STA_3&0x8000)            //3表示串口3的缓存标志
		{					   
			len=USART_RX_STA_3&0x3fff;//得到此次接收到的数据长度
			printf("\r\nthe news is:\r\n");
			HAL_UART_Transmit(&huart3,(uint8_t*)Rx_buff_3,len,1000);	//发送接收到的数据
